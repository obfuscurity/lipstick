  ...
  addAcknowledgement: function(event) {
    ...
    $.ajax({
      ...
      var newAlert = Ember.Object.create({
        type: 'success',
        message: 'acknowledged event ' + event.id + ' successfully',
        duration: 1
      })
      self.set('alerts', self.get('alerts').pushObject(newAlert));
      console.log(self.get('alerts'));
    }).fail(function(xhr, textStatus, errorThrown) {
