...
App.SitesShowController = Ember.ObjectController.extend({
  ...
  actions: {
    ...
    toggleAcknowledgeEvent: function(event) {
      if (event.get('acknowledged') == true) {
        this.removeAcknowledgement(event);
      } else {
        this.addAcknowledgement(event);
      }
    },
    ...
    checkEvent: function(event) {
      ...
      $.ajax({
        ...
        self.addAlert('success', 'scheduled forced check for event ' + event.id + ' successfully')
      }).fail(function(xhr, textStatus, errorThrown) {
        self.addAlert('danger', 'failed to schedule forced check for event' + event.id + ': ' + errorThrown);
      });
    },
    ...
    removeAlert: function() {
      this.set('alerts', this.get('alerts').slice(1,-1));
    }
  },

  addAlert: function(type, message) {
    var newAlert = Ember.Object.create({
      type: type,
      message: message,
      duration: type == 'success' ? 5 : 10
    });
    this.get('alerts').pushObject(newAlert);
  },

  addAcknowledgement: function(event) {
    var self = this;
    $.ajax({
      accepts: {json: 'application/json'},
      cache: false,
      ...
      self.addAlert('success', 'acknowledged event ' + event.id + ' successfully');
    }).fail(function(xhr, textStatus, errorThrown) {
      self.addAlert('danger', 'failed to acknowledge event' + event.id + ': ' + errorThrown);
    });
  },

  removeAcknowledgement: function(event) {
    var self = this;
    $.ajax({
      accepts: {json: 'application/json'},
      cache: false,
      ...
      event.set('acknowledged', !event.get('acknowledged'));
      self.addAlert('success', 'removed acknowledgement for event ' + event.id + ' successfully');
    }).fail(function(xhr, textStatus, errorThrown) {
      self.addAlert('danger', 'failed to remove acknowledgement for event' + event.id + ': ' + errorThrown);
    });
  },
  ...
  addDowntimeEvent: function(event) {
    ...
    $.ajax({
      ...
      self.addAlert('success', 'set downtime for event ' + event.id + ' successfully');
    }).fail(function(xhr, textStatus, errorThrown) {
      self.addAlert('danger', 'failed to set downtime for event ' + event.id + ': ' + errorThrown);
    });
  },
  ...
  cancelDowntimeEvent: function(event) {
    ...
    $.ajax({
      ...
      event.set('downtime', !event.get('downtime'));
      self.addAlert('success', 'removed downtime for event ' + event.id + ' successfully')
    }).fail(function(xhr, textStatus, errorThrown) {
      self.addAlert('danger', 'failed to remove downtime for event ' + event.id + ': ' + errorThrown);
    });
  }
})
