  ...
  actions: {
      ...
      sortEvents: function(column) {
          ...
          if (this.get("sortColumn") == column) {
              this.toggleProperty("sortReverse");
          }
          this.set('sortColumn', column);
          console.log(this.get('sortedEvents').map(function(event){return event[column];}))
      },
...
App.Site = Ember.Object.extend({
  ...
  hardEventsStatus: function() {
    return (this.get('hardEvents').length > 0) ? 'critical' : 'ok';
  ...
  sortedEvents: function() {
    ...
    if (sortColumn !== null) {
      if (sortColumn === 'duration') {
        mySortedEvents = this.get('events').sort(function(e1, e2) {
  ...
  afterModel: function(model) {
    var poll = function() {
      fetchSites().then(function(sites) {
        ...
        var idsInModel = model.mapBy('id');
        var idsFromServer = sites.map(function(s){ return s.id });
        var idsMissingFromServer = _.difference(idsInModel, idsFromServer);

        var removedIds = model.filter(function(site){
          var id = site.get('id');
          return idsMissingFromServer.indexOf(id) > -1;
        }).map(function(site){
          model.removeObject(site);
          return site.get('id');
        });

        sites = sites.filter(function(site){
          return removedIds.indexOf(site.id) > -1;
        });

        var newIds = _.difference(idsFromServer, idsInModel, idsMissingFromServer);
